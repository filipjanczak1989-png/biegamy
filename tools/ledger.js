...
//     node tools/ledger.js --samokontrola           → bariera PII + parser dat
//
...
function commity(od, doDnia) {
  const out = git(['log', '--since=' + od, '--until=' + doDnia + ' 23:59:59',
                   '--date=short', '--format=%h%ad%s']);
  return out.split('\n').filter(Boolean).map(l => {
    const [sha, data, temat] = l.split('');
