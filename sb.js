...
// ════════════════════════════════════════════════════════════════
// TEMP DEBUG (W2 diag, 2026-05-26) — mobile image-loading overlay.
// Aktywacja: dowolna strona z ?debug=img w URL. PURE OBSERVATION —
// NIE dotyka src/data-sp/observera, tylko czyta. Revert: usuń blok.
// ════════════════════════════════════════════════════════════════
(function _imgDebugOverlay() {
  try { if (!/[?&]debug=img\b/.test(location.search)) return; } catch (e) { return; }
  var box, list, open = true;
  ...
  function build() {
    ...
    box.style.cssText = [
      ...
      'max-width:min(94vw,420px)','max-height:55vh','overflow:auto',
      'background:#ff8c00','color:#000','font:13px/1.35 monospace',
      'border:2px solid #000','border-radius:8px','padding:6px 8px',
      'box-shadow:0 2px 12px rgba(0,0,0,.5)','-webkit-overflow-scrolling:touch'
    ].join(';');
    var hdr = document.createElement('div');
    hdr.style.cssText = 'display:flex;justify-content:space-between;align-items:center;' +
      'font-weight:bold;margin-bottom:4px;position:sticky;top:0;background:#ff8c00';
    var title = document.createElement('span'); title.textContent = 'IMG DEBUG';
    var x = document.createElement('button'); x.textContent = 'X';
    x.style.cssText = 'background:#000;color:#ff8c00;border:0;border-radius:4px;' +
      'font:bold 13px monospace;padding:1px 8px;cursor:pointer';
    x.onclick = function () { box.remove(); open = false; };
  ...
  function refresh() {
    ...
    var spLeft = document.querySelectorAll('[data-sp]').length;
    var rows = ['<div style="margin-bottom:3px">imgs:' + imgs.length +
      ' | ⏳sp:' + spLeft + ' | obs:' + (window._spObserverEnabled ? 'on' : 'OFF') + '</div>'];
    imgs.forEach(function (im, i) {
      var hasSp = im.hasAttribute('data-sp');
      var src = im.getAttribute('src') || '';
      var isPh = src.indexOf('data:image/svg+xml') === 0;
      var hidden = im.style.display === 'none';
      var nat = im.naturalWidth + '×' + im.naturalHeight;
      var label, status;
      if (hasSp)                                              { label = im.getAttribute('data-sp'); status = '⏳'; }
      else if (hidden && isPh)                                { label = '(hydration-fail)';        status = '🔴'; }
      else if (im.complete && im.naturalWidth > 0)            { label = src;                       status = '✅ ' + nat; }
      else if (im.complete && im.naturalWidth === 0 && src && !isPh) { label = src;                status = '❌'; }
      else                                                    { label = isPh ? '(placeholder)' : src; status = '…'; }
      rows.push('<div style="border-top:1px solid rgba(0,0,0,.25);padding:2px 0;word-break:break-all">' +
        '<b>' + status + '</b> #' + i + ' ' + esc(tail(label, 30)) + '</div>');
    });
