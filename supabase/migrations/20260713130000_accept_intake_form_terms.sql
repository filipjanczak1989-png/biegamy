-- FAZA 4 (Brama Regulaminu): accept_intake_form — przeniesienie zgody RODO do athletes
-- ─────────────────────────────────────────────────────────────────────────────
-- Intake (ankieta.html) MA realne checkboxy zgód → to PRAWDZIWA akceptacja, nie artefakt.
-- Dotąd accept_intake_form tworzył wiersz athletes BEZ terms_accepted_at → intake-athleci
-- lądowali z NULL (modal FAZY 2). Ta wersja przenosi zgodę: gdy consent_data_processing=true,
-- ustawia terms_accepted_at = consent_at (realny moment zgody; fallback created_at formularza).
--
-- ZAKRES: TYLKO ścieżka INSERT (tworzenie nowego athlety) — zgodnie z planem „przy tworzeniu
-- athletes". Ścieżka UPDATE (istniejący athleta tego samego trenera) NIE ruszа terms — istniejąca
-- zgoda nie degraduje (patrz FAZA 1 wariant B), a brak → modal FAZY 2. Cała reszta ciała funkcji
-- IDENTYCZNA z wersją produkcyjną (jedyny dopisek: kolumna+wartość terms_accepted_at w INSERT).
-- SECURITY DEFINER + search_path=public zachowane 1:1.
...
BEGIN
  ...
  IF v_athlete_id IS NULL THEN
    INSERT INTO athletes (
      ...
      pb_5k, pb_10k, pb_half, pb_marathon, goal, race_goals, gender,
      terms_accepted_at
    ) VALUES (
