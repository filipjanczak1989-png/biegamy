-- ── CEL 10 000 km — OSAD, nie pomiar ────────────────────────────────────────
-- Rachunek ZMIERZONY 13.08.2026 (nie szacowany):
--   mediana tygodniowa z 12 tygodni = 743 km (bez filtru isRunType 849;
--   najlepszy tydzien w historii 1014, wiec 1646 km/tydz NIE WYSTEPUJE w danych)
--   okno 15.08-20.09 = 37 dni = 5,3 tygodnia
--   bez nowych ludzi: ~3 930 km = 39% celu
--   zeby dobic 10 000: 1 891 km/tydz, czyli +154% wobec dzisiejszego tempa
-- Nieosiagalnosc bez lancuszka jest ZAMIERZONA: wyzwanie ma dawac powod do
-- zapraszania, nie mierzyc to, co wydarzy sie samo.
--
-- !! REALNE RYZYKO to pasek stojacy NISKO, nie dobity za wczesnie. Jesli po
--    dwoch tygodniach bedzie ponizej 15%, znaczy ze lancuszek nie zadzialal —
--    wtedy uczciwie obnizyc cel PUBLICZNIE, nie udawac.
--
-- !! KLUCZ CACHE nosi date poczatku okna. Zmiana okna BEZ zmiany klucza
--    oddawalaby wartosc policzona dla starego zakresu az do konca doby,
--    a nazwa klucza klamalaby na zawsze.
  ...
  if v_at is null or v_at < date_trunc('day', now()) then
    select coalesce(sum(l.distance_km), 0) into v_km
     ...
     where l.distance_km is not null
       and l.logged_at >= '2026-08-15' and l.logged_at < '2026-09-21'
