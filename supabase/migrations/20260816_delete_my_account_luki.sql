-- ─────────────────────────────────────────────────────────────────────────────
-- delete_my_account() — DOMKNIĘCIE LUK. 16.08.2026
--
-- POWÓD. Funkcja kasowała 24 tabele jawnie, a resztę zostawiała kaskadom FK
-- na `athletes`. Kaskady NIGDY nie odpalają, bo funkcja `athletes` NIE KASUJE —
-- ona go SCRUBUJE (nagrobek). Skutek: wszystko, co miało FK CASCADE, a nie
-- miało jawnego DELETE, przeżywało „usunięcie konta".
--
-- Zmierzone na produkcji 16.08.2026 (wierszy w tabeli ogółem):
--   wellness 80  ⚠️ resting_hr, hrv, sleep_secs, weight, readiness, spo2, vo2max
--   messages 432 · delivered_moments 126 · log_reactions 7 · log_comments 3
--   biegus_most 7 · peer_messages 0 · duels 0
--
-- ⚠️ ORAZ CZTERY TABELE BEZ FK — kaskada nie pomogłaby im nawet przy twardym
--    usunięciu wiersza `athletes`. Trzy z nich trzymają AUTH UID, nie
--    `athletes.id` (ten sam footgun co `nutrition_meals`). SPRAWDZONE, nie
--    założone — dopasowanie wierszy do obu kluczy:
--      game_events.athlete_id   199 wierszy → athletes.id 0 · user_id 103
--      radio_plays.user_id      319 wierszy → athletes.id 0 · user_id 312
--      client_errors.user_id    201 wierszy → athletes.id 0 · user_id 201
--      recipe_favorites         0 wierszy → klucza NIE DA SIĘ zmierzyć → oba
--
-- KONTA TRENERSKIE nie przechodzą tą drogą (RAISE na wejściu), więc
-- coaches / coach_week_templates / coach_workout_templates / radio_tracks
-- celowo zostają nietknięte.
-- ─────────────────────────────────────────────────────────────────────────────
